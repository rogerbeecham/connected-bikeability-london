...
For each commute we then search in `sampled_msoas` to attach point locations. This is achieved by taking each MSOA-MSOA OD pair and sampling MSOA point locations by destination according to the commute count of that OD pair.
...
# Rivers.
temp_rivers <- plot_data_temp %>% 
  filter(focus==1) %>% 
  st_drop_geometry() %>%  
  mutate(geometry=rivers$geometry) %>% 
  st_as_sf(sf_column_name = "geometry")
# Infrastructure
temp_infra <- plot_data_temp |>  
  filter(focus==1) |> 
  st_drop_geometry() |>   
  select(name, d_x,d_y) |>
  nest(data=c(name)) |> 
  mutate(
    geometry=map(data, ~infra_scheme$geometry)
  ) |> select(-data) |> unnest()  |> 
  st_as_sf(sf_column_name = "geometry")
...
plot_data_temp <- grid_real_sf |>  filter(type=="real") |> 
  right_join(temp_ods %>% left_join(simulated_data),  by=c("name"="o_village")) |> 
...
plot <- plot_data_temp %>% 
  ggplot(aes(index)) + 
  geom_histogram(aes(fill = ..x..), colour="#616161", size=.08) + 
  scale_fill_distiller(palette="PuBu", direction=1, guide="none") +
  labs(x="bikeability", y="village OD count") +
...
agg_map  <- grid_real_sf |>  filter(type=="real") |> 
  right_join(simulated_data |> group_by(d_village) |> summarise(index=mean(index)), by=c("name"="d_village"))  |>
  ggplot() +
...
# Summary statistics for trips over bridges.
crossbridge_od <- read.csv("data/cross_bridge_od.csv")

bikeability |>
  inner_join(crossbridge_od) |> 
  filter(bridge.name %in% c("London Bridge", "Blackfriars Bridge")) |> 
   summarise(mean=mean(cb_index), sd=sd(cb_index))
  group_by(bridge.name) |> 
  summarise(mean=mean(cb_index), sd=sd(cb_index))
```
